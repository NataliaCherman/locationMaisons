...
class GuesthouseController extends BaseController
{
    ...
    // Méthode pour afficher le formulaire d'ajout d'une maison d'hôte
    ...
    public function addguesthouse() 
    {
        ...
        $uploadDirectory = FCPATH . 'uploads/guesthouses/';
        ...
        if (isset($files['photos'])) {
            foreach ($files['photos'] as $file) {
                if ($file->isValid() && !$file->hasMoved()) {
                    ...
                    $relativePath = 'uploads/guesthouses/' . $randomName;
                    $photos[] = $relativePath;
                }
            }
        }
...
public function showDetailGuesthouse() {
    $guesthouseModel = new GuesthouseModel();
    $data['maisons'] = $guesthouseModel->findAll();
    return view('detailGuesthouse', $data);
}*/
...
public function editGuesthouse($id)
{
    ...
    if ($guesthouse) {
        return view('editGuesthouse', ['guesthouse' => $guesthouse]);
    } else {
...
/*public function updateGuesthouse($id)
{
    ...
    if (!empty($uploadedPaths)) {
        ...
        $data['photos'] = implode(',', $uploadedPaths);
    }
